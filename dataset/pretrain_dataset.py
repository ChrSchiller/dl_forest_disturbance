...
class PretrainDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        ### include indices?
        if self.indices or self.only_indices:
        ...
        if self.data_aug:
            ...
            if bool(random.getrandbits(1)):  # beginning
                ...
                # get value for 3/4*max_doy but min 2 years (=doy 365)
                if max_doy > 730:
                    # keep at least one year in the time series
                    latest_doy = int(max_doy - 730)

                    # get number/count of (non-zero!) observations until the latter value
                    ...
                    if (
                        ...
                        while ts[0, ts.shape[1]-1] > 730:
                            ts[:, ts.shape[1]-1] = ts[:, ts.shape[1]-1] - 730
        ...
        if self.seq_len > ts_length:
            ...
            if self.data_aug:
                ...
                noise = np.random.normal(0, .2, ts_origin[:ts_length, :].shape)
            ...
            if self.data_aug:
                ...
                ### data augmentation 4
                ### add a bit of noise (here: 1/10 of std of signal) to every value with respect to standard deviation
